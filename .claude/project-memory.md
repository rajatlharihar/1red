...
- `public/favicon.svg` — the verified pixel-perfect vector source of the "1red" logo mark (confirmed via pixel-sampling against the raster logo used elsewhere: identical shape AND color, `fill: red` = `#ff0000`). This is the asset to use for any task that needs to construct/animate/split the logo geometrically. It was previously used only as the browser favicon.
